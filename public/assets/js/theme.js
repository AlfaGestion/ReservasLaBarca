(function () {
    ...
    const MOON_ICON = [
        ...
        '<path d="M10.794 3.148a.217.217 0 0 1 .412 0l.387 1.162c.173.518.579.924 1.097 1.097l1.162.387a.217.217 0 0 1 0 .412l-1.162.387a1.73 1.73 0 0 0-1.097 1.097l-.387 1.162a.217.217 0 0 1-.412 0l-.387-1.162A1.73 1.73 0 0 0 9.31 6.593l-1.162-.387a.217.217 0 0 1 0-.412l1.162-.387a1.73 1.73 0 0 0 1.097-1.097zM13.863.099a.145.145 0 0 1 .274 0l.258.774c.115.346.386.617.732.732l.774.258a.145.145 0 0 1 0 .274l-.774.258a1.16 1.16 0 0 0-.732.732l-.258.774a.145.145 0 0 1-.274 0l-.258-.774a.145.145 0 0 0-.732-.732l-.774-.258a.145.145 0 0 1 0-.274l.774-.258c.346-.115.617-.386.732-.732z"/>',
        '</svg>',
    ].join('');

    function applyTheme(mode) {
        document.body.classList.toggle('theme-dark', mode === 'dark');
        const button = document.getElementById('themeToggleGlobal');
        if (!button) return;

        button.dataset.mode = mode;
        button.setAttribute('aria-label', mode === 'dark' ? 'Cambiar a modo claro' : 'Cambiar a modo oscuro');
        button.setAttribute('title', mode === 'dark' ? 'Cambiar a modo claro' : 'Cambiar a modo oscuro');
    ...
    function ensureToggleButton() {
        if (document.getElementById('themeToggleGlobal')) return;

        const button = document.createElement('button');
        button.type = 'button';
        button.id = 'themeToggleGlobal';
        button.className = 'theme-toggle-global';
        button.innerHTML = '<span class="theme-toggle-switch" aria-hidden="true"><span class="theme-toggle-knob">' + SUN_ICON + '</span></span>';
